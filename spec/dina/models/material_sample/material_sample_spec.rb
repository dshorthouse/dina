module Dina
  describe 'MaterialSample' do
    ...
    it "can have many prepared by" do
      material_sample = MaterialSample.new({ prepared_by: [ Person.new ]})
      expect(material_sample.prepared_by.size).to eq(1)
    end

    it "should raise an Exception if group is missing" do
      material_sample = MaterialSample.new({ group: nil, materialSampleName: "My sample" })
      expect { material_sample.save }.to raise_error(ObjectInvalid)
    end
    ...
    it "should raise an Exception if materialSampleType is invalid" do
      ...
      expect { material_sample.save }.to raise_error(PropertyValueInvalid)
    end
